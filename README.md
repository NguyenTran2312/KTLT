...
3. **`poly_approx.csv`**: Chứa giá trị P(x) tính bằng Horner và đạo hàm P'(x) tại mỗi điểm khi áp dụng hàm xấp xỉ.
4. **`poly_coefficients.csv`**:  Hệ số đa thức được tính toán bằng thuật toán Hồi quy bình phương tối thiểu (Khử Gauss) dựa trên đoạn tín hiệu tối ưu nhất.
